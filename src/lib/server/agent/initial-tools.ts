...
const capabilities:AgentToolDefinition={name:"get_service_capabilities",description:"현재 Agent에 실제 등록되어 사용할 수 있는 기능과 아직 연결되지 않은 기능을 확인한다.",inputSchema:emptyObjectSchema,requiresConnection:false,timeoutMs:DEFAULT_AGENT_LIMITS.toolTimeoutMs,maxResultCharacters:DEFAULT_AGENT_LIMITS.maxToolResultCharacters,sensitiveKeys:["password","encryptedPassword","apiKey","token"],async execute(){return {agentToolCalling:true,schemaSearch:true,knowledgeSearch:true,businessKnowledge:true,githubSourceExploration:true,readonlySqlValidation:true,readonlySqlExecution:"user_approval_required",availableTools:["get_service_capabilities","get_selected_database_context",...schemaAgentTools.map(tool=>tool.name),...knowledgeAgentTools.map(tool=>tool.name),...businessKnowledgeAgentTools.map(tool=>tool.name),...githubAgentTools.map(tool=>tool.name),...sqlAgentTools.map(tool=>tool.name)],conversationPersistence:true};}};
...
export function createInitialToolRegistry():ToolRegistry { return new ToolRegistry([capabilities,selectedDatabase,...schemaAgentTools,...knowledgeAgentTools,...businessKnowledgeAgentTools,...githubAgentTools,...sqlAgentTools]); }
