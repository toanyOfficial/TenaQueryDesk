...
import type { ToolSecurityMetadata } from "@/lib/server/security/security-types";
...
export type ToolContext = Readonly<{ userId: string; conversationId: string; connectionId: number | null; connection: TargetConnection | null; executionApproved?: boolean; securityActor?:import("@/lib/server/security/security-types").SecurityActor }>;
export type AgentToolDefinition = Readonly<{ name: string; description: string; inputSchema: JsonSchema; requiresConnection: boolean; timeoutMs: number; timeoutErrorCode?:string; maxResultCharacters: number; sensitiveKeys: ReadonlyArray<string>; security?:ToolSecurityMetadata; execute(context: ToolContext, input: Readonly<Record<string, unknown>>): Promise<unknown> }>;
export type ToolResult =
  | Readonly<{ ok: true; tool: string; data: unknown; meta: Readonly<{ durationMs: number; truncated: boolean; authorization?:Readonly<{reasonCode:string;appliedPolicies:ReadonlyArray<string>}>;redaction?:Readonly<{redacted:boolean;removedFieldCount:number;maskedTypes:ReadonlyArray<string>;policyId:string}> }> }>
...
export type RunAgentInput = Readonly<{ userId: string; connectionId: number | null; conversationId: string; userMessage: string; connection: TargetConnection | null; previousMessages?: ReadonlyArray<AgentMessage>; securityActor?:import("@/lib/server/security/security-types").SecurityActor }>;
