...
export function sanitizeSqlRows(rows:ReadonlyArray<Record<string,unknown>>, policies:ReadonlyMap<string,string>=new Map()){
 const blockedColumns=new Set<string>(),maskedColumns=new Set<string>();
 const safe=rows.map(row=>Object.fromEntries(Object.entries(row).map(([name,value])=>{const policy=policies.get(name.toLowerCase());if(policy==="secret"||policy==="restricted"||BLOCK.test(name)){blockedColumns.add(name);return [name,"[BLOCKED]"];}if((policy==="personal"||MASK.test(name))&&value!==null){maskedColumns.add(name);return [name,"[MASKED]"];}return [name,normalize(value)];})));
