...
export function AnalysisWorkspace() {
  ...
  return <main className="analysis-shell">
    ...
    <div className="analysis-grid"><AssistantPanel database={selected} messages={state.messages} question={state.question} disabled={assistantDisabled} submitting={submitting} error={state.queryStatus === "error" ? null : state.error} notice={state.notice ?? (selected && schema?.status !== "success" ? "최신 스키마가 없습니다. 관리 화면에서 스키마를 생성한 뒤 질문할 수 있습니다." : null)} onQuestionChange={(value) => dispatch({ type: "question", value })} onClear={() => dispatch({ type: "clearMessages" })} onSubmit={submitQuestion} onOpenHistory={openHistory} /><div className="analysis-right"><SqlEditorPanel database={selected} sql={state.sql} running={state.queryStatus === "running"} onChange={(value) => dispatch({ type: "sql", value })} onClear={() => dispatch({ type: "clearSql" })} onExecute={executeQuery} /><QueryResultPanel status={state.queryStatus} result={state.queryResult} error={state.error} queryExecutionLogId={state.queryExecutionLogId} onOpenHistory={openQueryHistory} /></div><HistoryDrawer open={historyOpen} status={historyStatus} items={historyItems} detailLoadingId={detailLoadingId} onClose={() => setHistoryOpen(false)} onReload={loadHistory} onSelect={restoreHistory} /><QueryHistoryDrawer open={queryHistoryOpen} status={queryHistoryStatus} items={queryHistoryItems} detailLoadingId={queryDetailLoadingId} onClose={() => setQueryHistoryOpen(false)} onReload={loadQueryHistory} onSelect={restoreQueryHistory} /></div>
  </main>;
}
