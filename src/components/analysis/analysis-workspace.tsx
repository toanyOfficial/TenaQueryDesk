...
function reducer(state: State, action: Action): State {
  ...
  if (action.type === "restore") return { ...state, messages: action.detail.messages.map(message=>({id:message.id,role:message.role,content:message.content,status:message.status,referencedTables:message.metadata?.references?.tables,businessKnowledge:message.metadata?.references?.businessKnowledge,github:message.metadata?.references?.github,runtime:message.metadata?.references?.runtime,ui:message.metadata?.references?.ui,warnings:message.metadata?.warnings,toolsUsed:message.metadata?.toolsUsed,sqlApplied:Boolean(message.metadata?.sql)})), sql: action.detail.workingState?.lastSql ?? state.sql, error: null, notice: `대화 “${action.detail.title}”을 불러왔습니다. 저장된 결과는 요약이며 원본 행은 재조회해야 합니다.` };
...
export function AnalysisWorkspace({sqlExecutionEnabled}:{sqlExecutionEnabled:boolean}) {
  ...
  useEffect(()=>{if(state.connectionId===null)return;const saved=window.localStorage.getItem(`tq-conversation-${state.connectionId}`);if(!saved)return;void (async()=>{try{const response=await fetch(`/api/analysis/history/${saved}`,{cache:"no-store"});if(!response.ok){window.localStorage.removeItem(`tq-conversation-${state.connectionId}`);return;}const body=await response.json() as {item?:AnalysisHistoryDetail};if(body.item?.connectionId===state.connectionId){dispatch({type:"restore",detail:body.item});setConversationId(body.item.id);}}catch{}})();},[state.connectionId]);
  ...
  async function submitQuestion() {
    ...
    try {
      const response = await fetch("/api/analysis/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ connectionId: state.connectionId, prompt, conversationId, requestId: crypto.randomUUID() }) });
      ...
      if (typeof (body as { conversationId?: unknown }).conversationId === "string") {const id=(body as { conversationId: string }).conversationId;setConversationId(id);window.localStorage.setItem(`tq-conversation-${state.connectionId}`,id);}
      dispatch({ type: "messages", value: [...nextMessages.slice(0, -1), { id: pending.id, role: "assistant", content: result.answer, status: "success", referencedTables: result.referencedTables, referencedDocuments: result.references?.documents, businessKnowledge: result.references?.businessKnowledge, github: result.references?.github, runtime: result.references?.runtime, ui: result.references?.ui, toolsUsed: result.references?.toolsUsed, assumptions: result.assumptions, warnings: result.warnings, sqlApplied, requestType: result.requestType, riskLevel: result.riskLevel, transactionGuidance: result.transactionGuidance, executionPlan: result.executionPlan }] });
  ...
  async function clearConversation(){if(conversationId){try{const response=await fetch(`/api/analysis/conversations/${conversationId}/reset`,{method:"POST"});if(!response.ok)throw new Error();}catch{dispatch({type:"analysisError",value:"서버 대화를 초기화하지 못했습니다."});return;}}if(state.connectionId!==null)window.localStorage.removeItem(`tq-conversation-${state.connectionId}`);setConversationId(null);dispatch({type:"clearMessages"});}
  async function restoreHistory(id: string) {
    ...
    try { const response = await fetch(`/api/analysis/history/${id}`, { cache: "no-store" }); if (response.status === 401) { router.replace("/login"); return; } if (!response.ok) throw new Error(); const body = await response.json() as { item?: AnalysisHistoryDetail }; if (!body.item || body.item.connectionId !== state.connectionId) throw new Error(); dispatch({ type: "restore", detail: body.item }); const activeId=body.item.status==="active"?body.item.id:null;setConversationId(activeId);if(activeId)window.localStorage.setItem(`tq-conversation-${state.connectionId}`,activeId);else window.localStorage.removeItem(`tq-conversation-${state.connectionId}`);setHistoryOpen(false); } catch { setHistoryStatus("error"); } finally { setDetailLoadingId(null); }
  }
  ...
  return <main className="analysis-shell">
    ...
    <div className="analysis-grid"><AssistantPanel database={selected} messages={state.messages} question={state.question} disabled={assistantDisabled} submitting={submitting} error={state.queryStatus === "error" ? null : state.error} notice={state.notice ?? (selected && schema?.status !== "success" ? "최신 스키마가 없습니다. Agent에게 현재 가능한 기능과 스키마 상태를 질문할 수 있습니다." : null)} onQuestionChange={(value) => dispatch({ type: "question", value })} onClear={()=>void clearConversation()} onSubmit={submitQuestion} onOpenHistory={openHistory} /><div className="analysis-right"><SqlEditorPanel executionEnabled={sqlExecutionEnabled} database={selected} sql={state.sql} running={state.queryStatus === "running"} referenceType={latestGuidance?.requestType ?? null} riskLevel={latestGuidance?.riskLevel ?? null} onChange={(value) => dispatch({ type: "sql", value })} onClear={() => dispatch({ type: "clearSql" })} onExecute={executeQuery} /><QueryResultPanel status={state.queryStatus} result={state.queryResult} error={state.error} queryExecutionLogId={state.queryExecutionLogId} onOpenHistory={openQueryHistory} /></div><HistoryDrawer open={historyOpen} status={historyStatus} items={historyItems} detailLoadingId={detailLoadingId} onClose={() => setHistoryOpen(false)} onReload={loadHistory} onSelect={restoreHistory} /><QueryHistoryDrawer open={queryHistoryOpen} status={queryHistoryStatus} items={queryHistoryItems} detailLoadingId={queryDetailLoadingId} onClose={() => setQueryHistoryOpen(false)} onReload={loadQueryHistory} onSelect={restoreQueryHistory} /></div>
  </main>;
}
