...
function reducer(state: State, action: Action): State {
  ...
  if (action.type === "queryRunning") return { ...state, queryStatus: "running", queryResult: null, error: null };
  if (action.type === "querySuccess") return { ...state, queryStatus: "success", queryResult: action.result, error: null };
  if (action.type === "queryEmpty") return { ...state, queryStatus: "empty", queryResult: action.result, error: null };
...
export function AnalysisWorkspace() {
  ...
  async function executeQuery() {
    ...
    try { const response = await fetch("/api/query/execute", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ connectionId: state.connectionId, sql: state.sql, analysisHistoryId: state.analysisHistoryId }) }); if (response.status === 401) { router.replace("/login"); return; } const body = await response.json() as { ok?: boolean; result?: QueryResult; error?: string; errorType?: string }; if (!response.ok || !body.ok || !body.result) throw new Error(body.error || "쿼리 실행에 실패했습니다."); dispatch({ type: body.result.rowCount === 0 ? "queryEmpty" : "querySuccess", result: body.result }); } catch (error) { dispatch({ type: "queryError", message: error instanceof Error ? error.message : "쿼리 실행에 실패했습니다." }); }
  }
  return <main className="analysis-shell">
    ...
    <div className="analysis-grid"><AssistantPanel database={selected} messages={state.messages} question={state.question} disabled={assistantDisabled} submitting={submitting} error={state.queryStatus === "error" ? null : state.error} notice={state.notice ?? (selected && schema?.status !== "success" ? "최신 스키마가 없습니다. 관리 화면에서 스키마를 생성한 뒤 질문할 수 있습니다." : null)} onQuestionChange={(value) => dispatch({ type: "question", value })} onClear={() => dispatch({ type: "clearMessages" })} onSubmit={submitQuestion} onOpenHistory={openHistory} /><div className="analysis-right"><SqlEditorPanel database={selected} sql={state.sql} running={state.queryStatus === "running"} onChange={(value) => dispatch({ type: "sql", value })} onClear={() => dispatch({ type: "clearSql" })} onExecute={executeQuery} /><QueryResultPanel status={state.queryStatus} result={state.queryResult} error={state.error} /></div><HistoryDrawer open={historyOpen} status={historyStatus} items={historyItems} detailLoadingId={detailLoadingId} onClose={() => setHistoryOpen(false)} onReload={loadHistory} onSelect={restoreHistory} /></div>
  </main>;
}
