...
import type { AnalysisMessage, DatabaseOption, GeneratedQueryResponse, QueryResult, QueryStatus, SchemaSummary } from "./types";
...
type Action = { type: "select"; id: number } | { type: "question"; value: string } | { type: "sql"; value: string } | { type: "clearMessages" } | { type: "clearSql" } | { type: "messages"; value: AnalysisMessage[] } | { type: "analysisError"; value: string | null };
...
export function AnalysisWorkspace() {
  ...
  async function submitQuestion() {
    ...
    try {
      ...
      const body = await response.json() as { ok?: boolean; result?: GeneratedQueryResponse; error?: string };
  ...
  return <main className="analysis-shell">
    ...
    <div className="analysis-grid"><AssistantPanel database={selected} messages={state.messages} question={state.question} disabled={assistantDisabled} submitting={submitting} error={state.error} notice={state.notice ?? (selected && schema?.status !== "success" ? "최신 스키마가 없습니다. 관리 화면에서 스키마를 생성한 뒤 질문할 수 있습니다." : null)} onQuestionChange={(value) => dispatch({ type: "question", value })} onClear={() => dispatch({ type: "clearMessages" })} onSubmit={submitQuestion} /><div className="analysis-right"><SqlEditorPanel database={selected} sql={state.sql} onChange={(value) => dispatch({ type: "sql", value })} onClear={() => dispatch({ type: "clearSql" })} /><QueryResultPanel status={state.queryStatus} result={state.queryResult} error={null} /></div></div>
  </main>;
}
