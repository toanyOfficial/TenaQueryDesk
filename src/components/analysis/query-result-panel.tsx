...
export function QueryResultPanel({ status, result, error }: Props) {
  return <section className="analysis-panel result-panel" aria-labelledby="result-heading">
    <header className="panel-bar"><div><span>REVIEW</span><h2 id="result-heading">SQL 실행 결과</h2></div><div className="result-meta">{result ? `${result.rowCount} rows · ${result.durationMs} ms` : "실행 전"}</div></header>
    <div className="result-scroll">
      ...
      {status === "success" && result && <table><thead><tr>{result.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{result.rows.map((row, index) => <tr key={index}>{result.columns.map((column) => <td key={column} title={String(row[column] ?? "NULL")}>{row[column] === null ? <em>NULL</em> : String(row[column] ?? "")}</td>)}</tr>)}</tbody></table>}
    </div>
