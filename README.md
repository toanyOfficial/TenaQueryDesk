...
관리자는 `/admin`에서 대상 MySQL의 접속정보를 등록합니다. `연결 키`는 비밀번호나 host가 아니라 연결 레코드와 스키마 파일 디렉터리를 구분하는 고유 식별자입니다. 영문 소문자·숫자로 시작하고 끝나는 64자 이하의 값에 점(`.`), 밑줄(`_`), 하이픈(`-`)을 사용할 수 있어 `dev.webpos.shop` 같은 도메인 형태도 허용합니다. 서비스는 자체 관리 테이블 `tq_db_connection`을 필요할 때 생성하고 비밀번호를 AES-256-GCM 암호문으로만 보관합니다. 등록 뒤 `스키마 파일 생성하기`를 실행하면 대상 DB의 `information_schema`를 읽어 서비스용 JSON 스키마 파일을 자동 생성하므로 사용자가 별도의 `schema.md`를 작성할 필요가 없습니다.
